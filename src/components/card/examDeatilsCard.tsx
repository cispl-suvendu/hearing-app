...
export default function ExamDeatilsCard({ exam }: ExamDeatilsCardProps) {
    ...
    return (
        <div>
            ...
            {showAddForm ? <Suspense fallback={<Skeleton />}><AssignExam exam={exam} reFetch={setReFetchCat} closeAddForm={handleShowForm} /></Suspense> : <Suspense fallback={<Skeleton />}><AssignExamCard assignedExam={assignedExam} setPage={setPage} setLimit={setLimit} pageLimit={limit} showPagination={true} /></Suspense>}
        </div>
    )
}
